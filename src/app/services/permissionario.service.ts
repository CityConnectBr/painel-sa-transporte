...
export class PermissionarioService extends BasicCrudService {
  ...
  indexVeiculos(permissionarioId: any): Observable<Veiculo[]> {
    return this.httpClient.get<any>(`${this.url}/${permissionarioId}/veiculos`, super.getHttpOptions)
      .pipe(
        first(),
  ...
  indexCondutores(permissionarioId: any): Observable<Condutor[]> {
    return this.httpClient.get<any>(`${this.url}/${permissionarioId}/condutores`, super.getHttpOptions)
      .pipe(
        first(),
        map(result => result['data'])
      )
